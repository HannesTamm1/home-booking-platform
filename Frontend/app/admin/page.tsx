  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-neutral-500">Real-time snapshot of the marketplace.</p>
      </div>
      ...
      <div className="mb-8 grid grid-cols-2 gap-4 lg:grid-cols-4">
        {[
          ...
          <div
            key={label}
            className={`rounded-2xl border p-5 ${alert ? "border-amber-200 bg-amber-50" : "border-neutral-200 bg-white"}`}
          >
            <p className="text-xs font-semibold uppercase tracking-widest text-neutral-500">{label}</p>
            <p className="mt-2 text-2xl font-bold tracking-tight">{value}</p>
            {href && alert && (
              <a href={href} className="mt-1 block text-xs text-amber-700 underline">View →</a>
            )}
          </div>
        ))}
      </div>
        ...
        <div className="flex h-32 items-end gap-3">
          {data?.weeklyBookings.map(({ date, count }) => (
            <div key={date} className="flex flex-1 flex-col items-center gap-1">
              ...
              <div
                className="w-full rounded-t-lg bg-rose-400 transition-all"
      ...
      {(data?.pendingListings ?? 0) > 0 && (
        <div className="mt-6 rounded-2xl border border-amber-200 bg-amber-50 p-5">
          ...
          <a href="/admin/listings" className="mt-1 text-sm text-amber-700 underline">
